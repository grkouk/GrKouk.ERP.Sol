...
public class DocumentTransactionServiceV2 : IDocumentTransactionService
{
    private readonly ApiDbContext _context;
    private readonly ILogger<DocumentTransactionService> _logger;
    private readonly IMapper _mapper;
    private readonly ITransactorTransactionService _transactorTransactionService;

    public DocumentTransactionServiceV2(ApiDbContext context, ILogger<DocumentTransactionService> logger, IMapper mapper, ITransactorTransactionService transactorTransactionService)
    {
        _context = context;
        _logger = logger;
        _mapper = mapper;
        _transactorTransactionService = transactorTransactionService;
    }
    ...
    public async Task<IActionResult> AddBuyDocument(BuyDocCreateAjaxDto docTrans)
    {
        ...
        try
        {
            ...
            if (transTransactorDef.DefaultDocSeriesId > 0)
            {
                ...
                var spTransactorCreateDto = new TransactorTransCreateDto
                {
                    TransDate = docTrans.TransDate,
                    TransTransactorDocSeriesId = transTransactorDefaultSeries.Id,
                    TransTransactorDocTypeId = transTransactorDefaultSeries.TransTransactorDocTypeDefId,
                    ...
                    Etiology = docTrans.Etiology
                };

                var serviceResult = await _transactorTransactionService.AddTransactorTransaction(spTransactorCreateDto, sectionCode);
            ...
            if (paymentMethod.AutoPayoffWay == SeriesAutoPayoffEnum.SeriesAutoPayoffEnumAuto)
            {
                ...
                if (autoPaySeriesId > 0)
                {
                    ...
                    var payoffCreateDto = new TransactorTransCreateDto
                    {
                        TransDate = docTrans.TransDate,
                        TransTransactorDocSeriesId = transTransactorPayOffSeries.Id,
                    ...
                    var payoffResult = await _transactorTransactionService.AddTransactorTransaction(payoffCreateDto, sectionCode);
        ...
        try
        {
            ...
            if (transTransactorDef.DefaultDocSeriesId > 0)
            {
                ...
                try
                {
                    ...
                    var serviceResult2 = await _transactorTransactionService.AddTransactorTransaction(spTransactorCreateDto, sectionCode);
                    if (!serviceResult2.Success)
                    {
                        if (ownsTransaction) await transaction.RollbackAsync();
                        return new BadRequestObjectResult(new { error = serviceResult2.ErrorMessage });
                    }

                    var tmpTrans2 = new TransactorTransaction
                    {
                        AmountNet = spTransactorCreateDto.AmountNet,
                        AmountFpa = spTransactorCreateDto.AmountFpa,
                        AmountDiscount = spTransactorCreateDto.AmountDiscount
                    };
                    ActionHandlers.TransactorFinAction(transTransactorDef.FinancialTransAction, tmpTrans2);
                    transToAttach.TransNetAmount = tmpTrans2.TransNetAmount;
                    transToAttach.TransFpaAmount = tmpTrans2.TransFpaAmount;
                    transToAttach.TransDiscountAmount = tmpTrans2.TransDiscountAmount;
                    transToAttach.TransExpensesAmount = 0;
                    _context.Entry(transToAttach).State = EntityState.Modified;
                }
                catch (Exception e)
