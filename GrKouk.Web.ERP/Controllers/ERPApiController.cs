...
namespace GrKouk.Web.ERP.Controllers
{
    ...
    public class ErpApiController : ControllerBase
    {
        ...
        {
            var query = _context.PaymentMethods.OrderBy(p=>p.Name).AsQueryable();
            ...
            try
            {
                ...
                if (WrItem != null)
                {
                    ...
                    if (companyMapping != null)
                    {
                        ...
                        {
                            itemId=WrItem.Id,
                            message = "Warehouse Item exists and already mapped to company"
                        });
                    }
                    var syncSrvResult = await _warehouseItemsManagementSrv.AddCompanyMappingToWarehouseItemAsync(WrItem.Id, companyId);
                ...
                var syncServiceResult = await _warehouseItemsManagementSrv.AddWarehouseItemAsync(warehouseItemToInsert);
        ...
        public async Task<IActionResult> ModifyCashierWarehouseItem([FromBody] CashierItemModifyRequest request)
        {
            ...
            try
            {
                ...
                var syncServiceResult = await _warehouseItemsManagementSrv.ModifyWarehouseItemAsync(itemToModify);
                if (syncServiceResult is null)
                {
                    return StatusCode(500, new
                    {
